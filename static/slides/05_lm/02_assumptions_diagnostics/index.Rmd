...
subtitle: 'The linear model:</br>.lightgrey[Assumptions, diagnostics, and interpretation]'
...
rmarkdown::render("./static/slides/05_lm/02_assumptions_diagnostics/index.Rmd")
xaringan::inf_mr()
```
...
read_chunk('./assets/scripts/assumptions_diagnostics.R')
```
...
---
background-image: url(https://media.licdn.com/mpr/mpr/AAEAAQAAAAAAAAd0AAAAJGFkMTNmMTg4LTg4MWMtNDAyNi1hMzdmLTY3OTA5YmUyMWFiNQ.jpg)
...
---
background-image: url(https://webondinc.com/wp-content/uploads/2018/01/apple.jpg)
...
```{r, create_assumptions_data, echo=FALSE, cache=FALSE}
```

```{r, fit_mods, echo=FALSE, cache=FALSE}
mod1 <- lm(y ~ x, data = assumptions_df)
mod2 <- lm(y_quad ~ x, data = assumptions_df)
```
...
assumptions_df %>% 
  ...
  geom_point(pch = 21, color = 'darkgrey', fill = 'darkred', 
             stroke = 1, size = 2) + 
  theme_test(base_family = 'Times', base_size = 20)
```
...
```{r, linear_model_print, cache=FALSE, results='asis', echo=FALSE}
summary(mod1$resid) %>% 
  tidy(.) %>% 
...
```{r, quad_model_print, cache=FALSE, results='asis', echo=FALSE}
...
```{r, homoskedasticity_linear, fig.width=14, echo=FALSE, fig.height=5}
...
```{r, homoskedasticity_quadratic, fig.width=14, echo=FALSE, fig.height=5}
...
library(DataCombine)
autocor_data <- data.frame(assumptions_df, resid_mod2=mod2$residuals)
...
cor.test(assumptions_df$x, mod1$residuals) # do correlation test 
```
...
cor.test(assumptions_df$x, mod2$residuals) # do correlation test 
```
...
```{r, normal_resids_linear, echo=FALSE, fig.height=5}
autoplot(mod1, which = 2) + my_theme()
```
...
```{r, normal_resids_quadratic, echo=FALSE, fig.height=5}
autoplot(mod2, which = 2) + my_theme()
```
...
```{r, leverage_linear, echo=FALSE, fig.height=5.5}
autoplot(mod1, which = 5) + my_theme()
```
...
```{r, leverage_quadratic, echo=FALSE, fig.height=5.5}
autoplot(mod2, which = 5) + my_theme()
```
...
noOut <- lm(y ~ x, data = assumptions_df[1:50, ])
...
<blockquote align='center' class="twitter-tweet" data-lang="de">
<a href="https://twitter.com/johncassil/status/964122022593945601"></a>
</blockquote>
...
<blockquote align='center' class="twitter-tweet" data-lang="de">
<a href="https://twitter.com/spcohen2/status/963982568482750464"></a>
</blockquote>
