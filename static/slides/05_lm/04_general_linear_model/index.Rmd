...
rmarkdown::render("./static/slides/05_lm/04_general_linear_model/index.Rmd")
xaringan::inf_mr()
```
...
read_chunk('./assets/scripts/general_linear_model.R')
```
...
ggplot(mtcars, aes(x = as.numeric(cyl), y = mpg)) + 
  ...
  labs(x = "cyl", y = "mpg") + 
  theme_test(base_size = 26, base_family = 'Times')
```
...
mtcars %>%
  ggplot(., aes(x = as.factor(cyl), y = mpg, color = as.factor(cyl))) + 
...
summary(mod_2)$coef %>% 
  tidy(.) %>% 
  rename(., Term = .rownames, `Std. Error` = Std..Error, p.value = Pr...t..) %>% 
...
vocab_sample <- sample_n(vocab_df, size = 200)

ggplot(vocab_sample, aes(x = age, y = vocab)) + 
...
vocab_age_mod <- lm(vocab ~ age, data = vocab_sample)
```
...
vocab_additive_mod <- lm(vocab ~ age + reader, data = vocab_sample)
```
...
vocab_sample %>% 
  ggplot(., aes(x = age, y = vocab, fill = reader)) + 
    geom_point(shape = 21, color = 'black', size = 3, alpha = 0.75) + 
    scale_fill_brewer(palette = "Set1") + 
    geom_abline(size = 1.5, color = 'grey40', 
                intercept = coef(vocab_age_reader_mod)[1], 
                slope = coef(vocab_age_reader_mod)[2]) + 
    geom_abline(size = 1.5, color = 'grey40',
                intercept = coef(vocab_age_reader_mod)[1] + 
                            coef(vocab_age_reader_mod)[3], 
                slope = coef(vocab_age_reader_mod)[2]) + 
    my_theme() + 
...
vocab_int_mod <- lm(vocab ~ age * reader, data = vocab_sample)
```
...
vocab_sample %>% 
  ggplot(., aes(x = age, y = vocab, fill = reader)) + 
    geom_point(shape = 21, color = 'black', size = 3, alpha = 0.75) + 
    scale_fill_brewer(palette = "Set1") + 
    geom_smooth(method = lm, se = F, size = 1.5, color = 'grey40') + 
    my_theme() + 
    theme(legend.position = c(0.2, 0.9))
```
...
best fit of the data (R<sup>2</sup> = 0.88). Overall, vocabulary size increased 
...
words +/- 57.63 se (t = 19.29, p < 0.001) per year. Frequent readers showed an 
