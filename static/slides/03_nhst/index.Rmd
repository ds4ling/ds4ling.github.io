...
rmarkdown::render("./static/slides/03_nhst/index.Rmd")
xaringan::inf_mr()
```
...
```{r, 'standard-normal', echo=FALSE, fig.height=6.5, fig.width=6.5, out.extra='style=float:right', fig.retina=2, warning=FALSE, cache=TRUE}
...
normalp1 <- ggplot(data = data.frame(x = c(5, 25)), aes(x)) +
  stat_function(fun = dnorm, n = 100, args = list(mean = 15, sd = 2), size = 1) + 
  labs(y = "", x = "") + 
  scale_y_continuous(breaks = NULL) + 
  scale_x_continuous(breaks = seq(5, 25, 2), 
                     labels = seq(5, 25, 2)) + 
  annotate("text", x = 10, y = 0.15, size = 7, label = TeX(pdf_tex1), 
           color = "red") + 
  my_theme() +
  theme(axis.line = element_line(colour = "black")) 

normalp2 <- ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  stat_function(fun = dnorm, n = 100, args = list(mean = 0, sd = 1), size = 1) + 
  labs(y = "", x = "") + 
  annotate("text", x = -2.5, y = 0.3, size = 7, label = TeX(pdf_tex3), color = "blue") +
  scale_y_continuous(breaks = NULL) + 
...
```{r, 'standard-normal2', echo=FALSE, fig.height=6.5, fig.width=6.5, out.extra="style='float:right'", fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}

normalp3 <- ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  stat_function(fun = dnorm, n = 100, args = list(mean = 0, sd = 1), size = 1) + 
  labs(y = "", x = "", title = 'Probability Density Function') + 
...
normalp4 <- ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  stat_function(fun = pnorm, n = 100, args = list(mean = 0, sd = 1), size = 1) + 
  labs(y = "", x = "", title = 'Distribution Function') + 
  annotate("text", x = -2.5, y = 0.75, size = 7, 
           label = TeX('$\\textit{D}(x)$')) +
...
```{r, 'standard-normal3a', echo=FALSE, fig.height=6.5, fig.width=6, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'standard-normal3b', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q1', echo=FALSE, fig.height=3.5, fig.width=7, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q2', echo=FALSE, fig.height=3.5, fig.width=7, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q3', echo=FALSE, fig.height=3.5, fig.width=7, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q4', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q5', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q6', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q7', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'q8', echo=FALSE, fig.height=3.5, fig.width=13, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, 'p6', echo=FALSE, fig.height=6, fig.retina=2, cache=TRUE}
p6 <- tibble(x = pop_iq) %>% 
  ggplot(., aes(x = x)) +
...
- The sample mean IQ we observe is 105.  We know that, even if the Trumps 
were not any different from the general public, we still might observe a 105 
...
```{r, 'iq-sample', cache=TRUE}
sample_n    <- 36  # Sample size
sample_mean <- 105 # xbar (trump family mean)
dist_mean   <- pop_mean
...
```{r, 'p7', echo=FALSE, fig.retina=2, fig.height=6, fig.width=8, include=FALSE, cache=TRUE}
p7 <- tibble(x = trumps) %>% 
  ggplot(., aes(x = x)) + 
    geom_density(color = 'black', fill = 'grey90') + 
     labs(x = 'IQ', y = 'Density', title = 'Density plot of IQ', 
...
p7 + geom_area(data = p7_build, aes(x = x, y = y, fill = trumps)) + 
     ...
     geom_segment(x = sample_mean + 1, xend = sample_mean + 2, 
                  y = 0.005, yend = 0.015, color = 'black', size = 0.75) + 
     geom_segment(x = sample_mean, xend = sample_mean, 
                  y = 0, yend = 0.035, color = 'black', size = 0.75) + 
     annotate("text", x = sample_mean, y = 0.045, label = "Sample\nmean", size = 4) + 
     annotate("text", x = sample_mean + 4.5, y = 0.02, size = 4,
...
```{r, 'p8', echo=FALSE, fig.retina=2, fig.height=7.5, fig.width=14, cache=TRUE}
p8 <- tibble(x = dist_z) %>% 
  ggplot(., aes(x = x)) +
    geom_density(color = 'black', fill = 'grey90') + 
     labs(x = 'IQ', y = 'Density', title = 'Density plot of IQ', 
...
p8 + geom_area(data = p8_build, aes(x = x, y = y, fill = trumps)) + 
     ...
     geom_segment(x = sample_z, xend = sample_z, 
                  y = 0, yend = 0.1, color = 'black', size = 0.75) + 
     geom_segment(x = sample_z + 0.25, xend = sample_z + 1, 
                  y = 0.025, yend = 0.05, color = 'black', size = 0.75) + 
     annotate("text", x = sample_z, y = 0.12, label = "Sample\nmean", size = 5) + 
     annotate("text", x = sample_z + 1.5, y = 0.075, size = 5,
              label = "Probability of observing\nlarger mean") + 
     theme(legend.position = 'bottom')
```
...
- In this case we see that the area to the right of **2** equals 1 - 0.9772, or 
`r 1 - 0.9772`.

- This implies that the probability of observing a sample IQ of 105 is .023.
...
- What we hypothesize is considered an *alternative* to a *null* hypothesis
	- **Null hypothesis** (H<sub>0</sub>): the difference between the 
	populations = 0
	- .blue[Alternative hypothesis] (H<sub>1</sub>): the difference 
...
prd_samp_long <- prd_samp %>% 
  gather(., key = test, val = score, -name)
```
