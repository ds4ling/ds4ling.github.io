...
rmarkdown::render("./static/slides/04_correlation/index.Rmd")
xaringan::inf_mr()
```
...
read_chunk('./assets/scripts/correlation.R')
```
...
---
background-image: url(https://www.tallentdmarketing.com/blog/wp-content/uploads/2016/03/target.jpg)
background-size: 600px
background-position: 90% 50%
...
background-image: url(https://media.licdn.com/mpr/mpr/AAEAAQAAAAAAAAl2AAAAJDlhMDIwYzEzLWMzYTYtNDdmZi04M2NlLTg2YmM2OGFmZTgyMg.jpg)
...
```{r, 'cor_examples', echo=FALSE, fig.retina=2, fig.width=15, fig.height=5, fig.align='center', cache=TRUE}
...
cor_plots <- function(plot_title, ylab = "y", r_xpos = 2, r_label) {
  list(
    ...
    theme_test(base_family = "Times", base_size = 18)
  )
}
...
```{r, echo=FALSE, fig.width=14, fig.retina=2, cache=TRUE}
poly_cor <- cor_data %>%
...
```{r, 'create_vocab_data', echo=FALSE}
```

```{r, 'vocab_plot', echo=FALSE, fig.width=13, fig.retina=2, fig.align='center', cache=TRUE}
...
```{r, 'create_vocab_table', echo=FALSE, warning=FALSE, cache=TRUE}
```

```{r, 'vocab_table0', results='asis', echo=FALSE, cache=TRUE}
vocab_table1 %>%
  select(., 1:3) %>% 
  slice(., 1:12) %>% 
...
```{r, 'vocab_table1', results='asis', echo=FALSE, cache=TRUE}
vocab_table1 %>%
  select(., 1:3) %>% 
  slice(., 1:12) %>% 
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
...
```{r, 'vocab_table2', results='asis', echo=FALSE, warning=FALSE, cache=TRUE}
vocab_table2 %>%
  select(., 1:3) %>% 
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
...
```{r, 'vocab_table3', results='asis', echo=FALSE, warning=FALSE, cache=TRUE}
vocab_table1 %>%
  select(., 1:4) %>% 
  slice(., 1:12) %>% 
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
...
```{r, 'vocab_table4', results='asis', echo=FALSE, warning=FALSE, cache=TRUE}
vocab_table1 %>%
  select(., 1:5) %>% 
  slice(., 1:12) %>% 
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
...
```{r, 'vocab_table5', results='asis', echo=FALSE, warning=FALSE, cache=TRUE}
vocab_table1 %>%
  select(., 1:6) %>% 
  slice(., 1:12) %>% 
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
...
```{r, 'vocab_table6', results='asis', echo=FALSE, warning=FALSE, cache=TRUE}
vocab_table2 %>%
  kable(., format = 'html', full_width = T, escape = FALSE, align = 'r') %>% 
  row_spec(13, bold = T, color = "white", background = "white") %>%
...
```{r, 'hand_calc_r', echo=TRUE, cache=TRUE}
...
```{r, echo=FALSE, fig.retina=2, fig.width=6.5, fig.height=4.5}
...
<iframe src="http://www.tylervigen.com/spurious-correlations" style="border:none;" height="600" width="100%"></iframe>
...
Copy and paste this dataset into an r-script.
