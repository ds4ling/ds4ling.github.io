...
rmarkdown::render("./static/slides/02_distributions/index.Rmd")
xaringan::inf_mr()
```
...
layout: false
class: center
background-image: url(../../sources/img/workflow/datascience_workflow.png)
...
```{r, 'normal_distribution1', cache=TRUE, echo=FALSE, fig.retina=2, fig.height=5}
...
```{r, 'normal_distribution2', echo=FALSE, fig.retina=2, fig.height=5, cache=TRUE}
...
```{r, normal-distributions3, echo=FALSE, fig.retina=2, fig.height=5, fig.width=3.5, cache=TRUE}
...
```{r, normal-distributions4, echo=FALSE, fig.retina=2, fig.height=5, cache=TRUE}
...
```{r, 'uniform_distribution', echo=FALSE, fig.retina=2, fig.height=5, cache=TRUE}
...
```{r, 'non_normal_distributions', echo=FALSE, fig.retina=2, fig.height=4.5, fig.width=14, cache=TRUE}

my_skewed_x <- fGarch::rsnorm(1000, mean = 2.5, sd = 1.5, xi = 3.5)

p3 <- tibble(x = my_skewed_x) %>%
  ggplot(., aes(x = x)) + 
    geom_histogram(binwidth = 0.25, fill = 'lightgrey', color = 'black', 
                   aes(y = ..density..)) + 
    geom_density(bw = 0.5) + 
...
```{r, 'p123', echo=FALSE, fig.retina=2, fig.height=8, fig.width=14, fig.align='center', cache=TRUE}
# combine vectors to plot all three distributions on top of each other 
combined_dists <- tibble(normal = my_normal_x, 
                         uniform = my_uniform_x, 
                         skewed = my_skewed_x, 
                         outliers = my_outlier_x) %>% 
  gather(., key = type, val = val)
...
```{r, 'qqplot-build', echo=FALSE, include=FALSE, cache=TRUE}
...
```{r, 'qqplots1', echo=FALSE, fig.retina=2, fig.width=8, fig.height=9, include=FALSE, cache=TRUE}
p1 / qq1 
```
...
```{r, 'qqplots234', echo=FALSE, fig.retina=2, fig.width=14, fig.height=8.5, cache=TRUE}
...
```{r, 'exam-scores1', echo=FALSE, fig.retina=2, fig.width=14, fig.height=6, cache=TRUE}
...
```{r, 'ex-descriptives1', results='asis', echo=FALSE, cache=TRUE}
descriptives_a %>% select(-group) %>% kable(format = 'html')
```

---

```{r, 'exam-scores2', echo=FALSE, fig.retina=2, fig.width=14, fig.height=7, cache=TRUE}
...
```{r, 'ex-descriptives2', results='asis', echo=FALSE, cache=TRUE}
...
```{r, 'exam-scores-std1', echo=FALSE, fig.retina=2, fig.width=14, fig.height=7, cache=TRUE}
...
```{r, 'ex-print1', results = 'asis', cache=TRUE, echo=FALSE}
scores %>% 
  ungroup(.) %>%
  filter(., group == 'Class_A') %>% 
...
```{r, 'ex-print2', results = 'asis', cache=TRUE, echo=FALSE}
scores %>% 
  ungroup(.) %>%
  filter(., group == 'Class_B') %>% 
  select(-important, -group) %>% 
