...
subtitle: 'The linear model:</br>.lightgrey[Bivariate regression]'
...
```{r holder, eval=FALSE, echo=FALSE}
rmarkdown::render("./static/slides/05_lm/01_bivariate_regression/index.Rmd")
xaringan::inf_mr()
```
...
read_chunk('./assets/scripts/lm.R')
```
...
the *slope* and <blue>a</blue> is the point where the line crosses the 
...
imperf_plot1 <- ggplot(imperf_points1, aes(x = x, y = y)) + 
  ...
  geom_abline(intercept = 0.5, slope = 1.1, 
              lty = 2, color = "darkred") + 
  geom_segment(aes(xend = imperf_points1$x, yend = imperf_points1$fitted1), 
               lty = 3, size = 1) + 
  geom_point(color = 'blue', fill = 'grey70', pch = 21, size = 6) + 
  theme_bw(base_size = 20, base_family = 'Times')
...
imperf_plot2 <- ggplot(imperf_points1, aes(x = x, y = y)) + 
  ...
  geom_abline(intercept = 0.25, slope = 0.5, 
              lty = 2, color = "darkred") + 
  geom_segment(aes(xend = imperf_points1$x, yend = imperf_points1$fitted2), 
               lty = 3, size = 1) + 
  geom_point(color = 'blue', fill = 'grey70', pch = 21, size = 6) + 
  theme_bw(base_size = 20, base_family = 'Times')
...
imperf_plot1 + imperf_plot2 + imperf_plot3 + plot_annotation(tag_levels = '1', 
                                                             tag_suffix = ".")
...
imperf_plot_bestfit + annotate("text", x = 1, y = 3, size = 7,
                               label = TeX("$\\hat{y} = 1 + 0.5x$"))
```
...
- Literally calculated as **r**^2 = r<sup>2</sup>
...
ggplot(., aes(x = x, y = y, fill = as.factor(varexp))) + 
  ...
  theme_bw(base_size = 20, base_family = 'Times') + 
  annotate("text", x = 1, y = 2.2, label = TeX("$y_1$"), fontface = 5, size = 6) + 
  annotate("text", x = 2, y = 0.8, label = TeX("$y_2$"), fontface = 5, size = 6) +
  annotate("text", x = 3, y = 3.2, label = TeX("$y_3$"), fontface = 5, size = 6)
