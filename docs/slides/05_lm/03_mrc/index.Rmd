...
rmarkdown::render("./static/slides/05_lm/03_mrc/index.Rmd")
xaringan::inf_mr()
```
...
read_chunk('./assets/scripts/mrc.R')
```
...
<a href="https://twitter.com/corycurl/status/966725487702790144"></a>
</blockquote>
...
--
  ...
  - This biases the parameter estimates (e.g., b-weights) for the 
  other predictors
...
<iframe src="./assets/html/threejs1.html" width='1000' height='600' style="border:none;"></iframe>
...
b <- coef(mod)
cat(sprintf("$$mpg = %.02f + %.02f wt + %.02f drat$$", b[1], b[2], b[3]))
```
--
...
- Basically, test the model once with everything and then test the model 
...
---
class: title-slide-section-red
background-image: url(https://scontent-lga3-1.cdninstagram.com/vp/c5d834b393a4e7e578253c8e2ccee338/5B3F62C1/t51.2885-15/e35/12716470_1688916864717116_2047131572_n.jpg)
background-size: 400px
background-position: 90% 50%
...
- It might be worthwhile see if it is a good idea by doing the math first
...
```{r, alpha_slippage, warning=FALSE, echo=FALSE, fig.height=5}
...
tibble(x = 1:100, y = error_rate) %>% 
  ggplot(., aes(x = x, y = y)) + 
   geom_point(pch = 21, fill = 'lightblue', size = 7) + 
