...
rmarkdown::render("./static/slides/06_glm/index.Rmd")
xaringan::inf_mr()
```
...
source('./assets/scripts/logistic_regression.R')
source('./assets/scripts/poisson_regression.R')
```
...
```{r, cache=F, echo=F, fig.height=5, fig.width=8, fig.align='right'}
log_plot
```
</div>
...
```{r, 'lin_log_comp', cache=FALSE, echo=FALSE, fig.width=14, fig.height=9}
mod_lin <-  lm(resp ~ vot, data = logistic_df)
mod_log <- glm(resp ~ vot, data = logistic_df, family = "binomial")

p_lin <- ggplot(logistic_df, aes(x = vot, y = resp)) + 
  geom_jitter(pch = 21, alpha = 0.1, height = 0.05, width = 6, color = 'blue') + 
  #stat_summary(fun.data = mean_cl_boot, geom = 'pointrange') + 
  #stat_summary(fun.y = mean, geom = 'point', pch = 21, 
  #             size = 6, fill = 'white') + 
  geom_smooth(method = lm, fullrange = TRUE, size = 1) + 
...
p_log <- ggplot(logistic_df, aes(x = vot, y = resp)) + 
  ...
  geom_smooth(method = glm, method.args = list(family = "binomial"), 
              fullrange = T, size = 1) +
  stat_summary(aes(y = fitted(mod_log)), fun.y = mean, geom = "line") + 
  coord_cartesian(ylim = c(-0.2, 1.2), xlim = c(-80, 80)) + 
...
```{r, vot_plot, echo=FALSE, fig.width=8, fig.height=8}
ggplot(logistic_df, aes(x = vot, y = resp)) + 
  geom_hline(yintercept = 0.5, lty = 3, size = 0.7) + 
  geom_jitter(pch = 21, alpha = 0.2, height = 0.05, width = 6, color = 'darkred') + 
  stat_summary(fun.data = mean_cl_boot, geom = 'pointrange') + 
  stat_summary(fun.y = mean, geom = 'point', pch = 21, 
               size = 6, fill = 'white') + 
  geom_smooth(method = glm, method.args = list(family = "binomial"), 
  ...
  theme_test(base_size = 24, base_family = "Times")
```
...
```{r, 'lin_poisson_comp', cache=FALSE, echo=FALSE, fig.width=14, fig.height=8.5}
mod_lin_count <-  lm(units ~ temp, data = poisson_df)
mod_poisson   <- glm(units ~ temp, data = poisson_df, family = poisson(link = "log"))

p_lin_count <- ggplot(poisson_df, aes(x = temp, y = units)) + 
  geom_point(pch = 21) + 
  geom_smooth(method = lm, fullrange = TRUE, size = 2) + 
  #coord_cartesian(ylim = c(0, 1000), xlim = c(30, 112)) +
  scale_y_continuous(breaks = NULL, labels = NULL) + 
  scale_x_continuous(breaks = NULL, labels = NULL) + 
  labs(y = "", x = "") + 
  my_theme()

p_poisson <- ggplot(poisson_df, aes(x = temp, y = units)) + 
  geom_point(pch = 21) + 
...
ggplot(poisson_df, aes(x = temp, y = units)) + 
  geom_point(pch = 21, fill = 'grey80') + 
  geom_smooth(method = glm, method.args = list(family = "poisson")) + 
  scale_color_brewer(palette = "Set1", name = "") + 
  labs(x = "Temperature", y = "Units ice cream sold") + 
  theme_test(base_size = 20, base_family = "Times")
```
...
ggplot(poisson_df, aes(x = temp, y = units, color = city)) + 
  geom_point(pch = 21, fill = 'grey80') + 
  geom_smooth(method = glm, method.args = list(family = "poisson")) + 
  scale_color_brewer(palette = "Set1", name = "") + 
  labs(x = "Temperature", y = "Units ice cream sold") + 
...
ggplot(poisson_df, aes(x = (temp - mean(temp)), y = units, color = city)) + 
  geom_point(pch = 21, fill = 'grey80') + 
  geom_smooth(method = glm, method.args = list(family = "poisson")) + 
...
- At the average temperature (`r mean(poisson_df$temp)`), ice cream  
